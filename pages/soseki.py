...
#タイトルファイルの読み込み
titles = get_list_from_file('./all_data/out_000148/book-titles148.txt')
...
#csvファイルの読み込み
df2=pd.read_csv("./all_data/out_000148/tfidf148.csv")

#類似度を格納する辞書
data_dict={}

#tf-idfやカウントモデルの読み込み
with open("./all_data/out_000148/tfidf148model.pkl", 'rb') as f:
    vectorizer = pickle.load(f)

with open("./all_data/out_000148/count148model.pkl", 'rb') as f:
...
with st.form(key='profile form'):
    ...
    if submit_btn:
        for x in range(len(dic2)):
            ...
            st.write("あらすじ")
            st.write(choice("./all_data/generate/148/%d.txt" % dic2[x][0]))
            st.markdown("---") #区切り線